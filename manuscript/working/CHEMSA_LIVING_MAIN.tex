...
% The current seed source references linewidth_hierarchy.png, but that generated
% figure asset is not presently stored in the repository or supplied source set.
% The living manuscript therefore remains compilable with an explicit working
% placeholder. Adding the real figure later requires no prose change.
\IfFileExists{linewidth_hierarchy.png}{}{%
  \renewcommand{\includegraphics}[2][]{%
    \fbox{\begin{minipage}{0.94\linewidth}\centering
    \textbf{Working figure placeholder}\\[0.4em]
    \texttt{linewidth\_hierarchy.png} is referenced by the current manuscript seed but is not yet staged in the living-manuscript workspace.\\
    This placeholder is not a publication figure.
    \end{minipage}}%
  }%
}
...
\input{sections/02_method_generator_first}
\input{sections/03_scope_boundary}
\input{sections/04_damping_coordinate}
\input{sections/05_linewidth_procedure}
\input{sections/06_rhco2acac_worked_trajectory}
\input{sections/07_published_system_admission}
\input{sections/08_phase_mixing_scope}
\input{sections/09_barrier_height_rate_atlas}
\input{sections/10_discussion}
\input{sections/11_limitations}
\input{sections/12_conclusion_and_availability}
